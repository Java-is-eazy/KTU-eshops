...
import { BrowserRouter as Router, MemoryRouter, ReactRouterDOM, useNavigate,Routes,Route} from 'react-router-dom';
...
test('renders all input fields', () => {
  ...
  test('shows error message when email is invalid (without @)', () => {
    ...
    const submitButton = getByText('Payment');
  
    fireEvent.change(fullNameInput, { target: { value: 'John Doe' } });
    fireEvent.change(emailInput, { target: { value: 'invalidemail' } });
    fireEvent.click(submitButton);
  
    const errorMessage = getByText('Please fill in all required fields!');
  ...
  describe('CheckoutPage component', () => {
    it('should navigate to payment page on successful form submission', async () => {
      ...
      render(
            <MemoryRouter initialEntries={['/checkout/23']}> {/* Simulating navigating to /checkout/123 */}
                <Routes>
      ...
      fireEvent.click(submitButton);
      await waitFor(() => fireEvent.click(screen.getByText('Card')), { timeout: 500 });
;
      expect(mockedNavigate).toHaveBeenCalledWith(expect.stringContaining('/payment/'), expect.anything());
    });
  });